...
wet_day_mm <- 1
p_extreme  <- 0.95
K_joint    <- 2   # stations extreme in one ISO week

# time cross validation folders
splits <- list(
...
# brier and logloss function for validation
...
# calibration plot joint. pooled plots (of the three folds)
...
# ranking based on brier logloss auc
ranked <- results_df %>%
  group_by(fold) %>%
  mutate(
