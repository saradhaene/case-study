...
# Output directory
OUT_DIR <- file.path(DATA_DIR, "outputs_asturias_ALL")
...
if (RUN_3FOLD_CV) {
  ...
  write.csv(cv_table, file.path(OUT_DIR, "cv3fold_scores_Asturias.csv"), row.names=FALSE)
  cat("\nSaved 3-fold CV scores to:", file.path(OUT_DIR, "cv3fold_scores_Asturias.csv"), "\n")
  ...
  write.csv(cv_mean, file.path(OUT_DIR, "cv3fold_mean_scores_Asturias.csv"), row.names=FALSE)
  cat("Saved 3-fold mean scores to:", file.path(OUT_DIR, "cv3fold_mean_scores_Asturias.csv"), "\n")
